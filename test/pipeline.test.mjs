...
test("EPUB 解析产出卡片（含 ? 标签与 FSRS 字段）", async () => {
	const r = await pipeline.runImport(bytes, "demo.epub", {});
	assert.ok(r.tiddlers.length > 1, "应有文档页 + 卡片");
	const cards = r.tiddlers.filter((t) => Array.isArray(t.tags) && t.tags.includes("?"));
...
test("NCX 锚点切分：面包屑包含目录小节标题", async () => {
	const r = await pipeline.runImport(bytes, "demo.epub", {});
	const trails = r.tiddlers.filter((t) => Array.isArray(t.tags) && t.tags.includes("?")).map((t) => String(t["tidme.breadcrumb"] || ""));
...
test("确定性 ID：同一面包屑与序号派生稳定", async () => {
	const r1 = await pipeline.runImport(bytes, "demo.epub", {});
	const r2 = await pipeline.runImport(bytes, "demo.epub", {});
	const id1 = r1.tiddlers.filter((t) => Array.isArray(t.tags) && t.tags.includes("?")).map((t) => t["tidme.id"]);
	const id2 = r2.tiddlers.filter((t) => Array.isArray(t.tags) && t.tags.includes("?")).map((t) => t["tidme.id"]);
