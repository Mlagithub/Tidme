/*
import-split.test.mjs — 通用切分器 runSplit 集成测试（node:test）
...
import { loadImportBundle } from '../helpers/jsdom-env.mjs';
...
test('split: markdown 围栏代码为原子块（不切分、原样保留）', async () => {
  const r = await importBundle.runSplit({ text: MD_SAMPLE, title: 'MD测试', type: 'text/markdown' });
...
test('split: markdown 表格为原子块', async () => {
  const r = await importBundle.runSplit({ text: MD_SAMPLE, title: 'MD表格', type: 'text/markdown' });
  const cards = cardsOf(r);
...
test('split: wikitext `!` 标题建树', async () => {
  const wt = '! 章一\n\n段落甲。\n\n!! 节一\n\n段落乙。\n\n! 章二\n\n段落丙。';
  const r = await importBundle.runSplit({ text: wt, title: 'WT测试', type: 'text/vnd.tiddlywiki', minChars: 0 });
  const cards = cardsOf(r);
...
test('split: HTML h1-h6 建树', async () => {
  ...
  const r = await importBundle.runSplit({ text: html, title: 'HTML测试', type: 'text/html', minChars: 0 });
  const cards = cardsOf(r);
...
test('split: 无 type 自动探测（markdown / wikitext / html）', async () => {
  const md = await importBundle.runSplit({ text: '# 标题\n\n正文。', title: 'sniff-md' });
  assert.equal(md.format, 'markdown');
  const wt = await importBundle.runSplit({ text: '! 标题\n\n正文。', title: 'sniff-wt' });
  assert.equal(wt.format, 'wikitext');
  const html = await importBundle.runSplit({ text: '<html><body><h1>标题</h1><p>正文</p></body></html>', title: 'sniff-html' });
  assert.equal(html.format, 'html');
});

test('split: docId 由标题派生，同一标题+内容重切分稳定', async () => {
  const input = { text: '# 甲\n\n内容。\n\n## 乙\n\n更多内容。', title: '稳定性测试', type: 'text/markdown' };
  const r1 = await importBundle.runSplit(input);
  const r2 = await importBundle.runSplit(input);
...
test('split: 不再生成自动阅读牌组（topic 走阅读列表，不进牌组体系）', async () => {
  const r = await importBundle.runSplit({ text: '# 书名\n\n内容。', title: '自动牌组书', type: 'text/markdown' });
...
test('split: EPUB3 nav-only 书籍按 nav 目录切分', async () => {
  ...
  const r = await importBundle.runImport(bytes, 'demo3.epub', { minChars: 0 });
  const cards = cardsOf(r);
...
test('SM A-Factor: 节卡按篇幅启发式携带 tidme.afactor（短文 2.0 / 长文 1.3）', async () => {
  const short = await importBundle.runSplit({ text: '# 短文\n\n' + '短文内容。'.repeat(40), title: '短文A', type: 'text/markdown' });
  ...
  const long = await importBundle.runSplit({ text: '# 长文\n\n' + '长文内容段落。'.repeat(700), title: '长文B', type: 'text/markdown' });
