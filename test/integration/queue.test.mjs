...
test('调度: 学习会话 core/session — set/get/removeFrom/advance（cur 之后推进，跳未来排期；交错含 topic）', () => {
  ...
  const mk = (t, o = {}) =>
    wiki.addTiddler({
      title: t,
      'tidme.kind': o.kind || 'item',
      state: o.state ?? '0',
      due: o.due !== undefined ? T(o.due) : T(now),
      caption: t,
      text: 'x',
    });
  mk('sItem1');
  mk('sItem2');
  mk('sTopic', { kind: 'topic', due: now - 1000 });
  mk('sItem3');
  mk('sFuture', { state: '2', due: now + 5 * 86400000 });
  mk('sItem4');
...
test('调度: schedulable-next filter operator — startstudy 的 session_next 语义（单一实现）', () => {
  ...
  mk('oItem1');
  mk('oItem2');
  mk('oTopic', { kind: 'topic', due: now - 1000 });
  mk('oItem3');
  mk('oFuture', { state: '2', due: now + 86400000 });
...
test('词书牌组页（learning-package legacy kind=topic）不混入学习流与阅读队列', () => {
  wiki.addTiddler({
    title: '$:/Deck/IELTS_T',
    tags: ['$:/tags/TidmeDeck'],
    caption: '雅思词汇',
    'tidme.kind': 'topic', // learning-package 词书页的 legacy 字段：词卡管理单元，非阅读材料
    'tidme.doc': 'IELTS_T',
  });
  ...
  mkCard('qItem', { kind: 'item', state: '2', due: new Date(Date.now() - 3600000) });
