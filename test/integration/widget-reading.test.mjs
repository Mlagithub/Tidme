...
import { collectText, collectButtons, renderWidget as renderWidgetBase } from "../helpers/fake-dom.mjs";
...
test.beforeEach(async () => {
	reset();
	F = await makeBookFixture(wiki, pipeline);
});
...
test("align: 重复导入（A）——同内容再导入不覆盖 SRS 进度", async () => {
	...
	const r2 = await pipeline.runSplit({ text: "# 重导书\n\n甲内容。\n\n## 乙\n\n乙内容。", title: "重导书", type: "text/markdown", minChars: 0 });
