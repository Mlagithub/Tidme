...
test('parse/pdf: LLM-OCR 请求构建与响应解析', () => {
  ...
  assert.equal(parsePdf.ocrTiddlerTitle('Tidme/Docs/书', 3), 'Tidme/Docs/书/ocr-p3');
});

test('core/pdf-ops: createPdfDoc 整本落库 —— 二进制/文档页就位并入队（不切分）', async () => {
  const r = await pdfOps.createPdfDoc(wiki, { docTitle: '未来简史', dataB64: 'JVBERi0xLjK=' });
  assert.equal(ns.NS_ASSETS, 'Tidme/Assets/');
  assert.equal(r.pdfTitle, 'Tidme/Assets/未来简史');
...
test('core/pdf-ops: 空 dataB64 拒绝落库；reattachPdfBinary 原位恢复二进制', async () => {
  ...
  await assert.rejects(
    pdfOps.createPdfDoc(wiki, { docTitle: '空二进制书', dataB64: '' }),
    /dataB64/,
  );
  // 构造存量损坏：二进制条目 text 为空（服务端 0 字节 .pdf 在客户端的形态）
  const r = await pdfOps.createPdfDoc(wiki, { docTitle: '重绑测试书', dataB64: 'JVBERi0xLjQK' });
  ...
  assert.equal(wiki.getTiddler(r.docTitle).fields['tidme.asset'], r.pdfTitle, '文档页关联不变');
...
test('core/pdf-ops + doc-ops: 清理阅读材料级联 PDF 二进制与 OCR 转写页', async () => {
  const r = await pdfOps.createPdfDoc(wiki, { docTitle: '待清理书', dataB64: 'JVBERg==' });
...
test('pdf-reader: 二进制缺失或为空时状态条提供重新绑定入口', async () => {
  const r = await pdfOps.createPdfDoc(wiki, { docTitle: '缺二进制渲染书', dataB64: 'JVBERi0xLjQK' });
...
test('pdf-reader: 工具栏结构 —— 缩放/翻页/全屏齐备（无目录：不切分）', () => {
  ...
  wiki.addTiddler({
    title: 'Tidme/Docs/裸工具栏书',
    tags: ['tidme-doc'],
    'tidme.kind': 'topic',
    'tidme.format': 'pdf',
    'tidme.asset': 'Tidme/Assets/裸工具栏书',
    'tidme.structure': 'continuous',
    'tidme.doc': 'dpx2',
    text: '<$tidme-pdf-reader/>',
  });
  wiki.addTiddler({ title: 'Tidme/Assets/裸工具栏书', type: 'application/pdf', text: 'JVBERi0xLjK=' });
...
test('pdf-reader: resolvePdfContext 路径回退（分节卡向上解析文档页）', () => {
  const pdfWidgetMod = mod('read/widgets/pdf-reader.js');
  const resolve = pdfWidgetMod.resolvePdfContext;

  const docRoot = 'Tidme/Docs/极简测试书';
  const secTitle = `${docRoot}/01 第一节`;
  const pdfTitle = 'Tidme/Assets/极简测试书';
  ...
  assert.equal(ctx1.pdfTitle, pdfTitle, '通过路径父级 Tidme/Docs/极简测试书 找到 pdfTitle');
  assert.equal(ctx1.docPageTitle, docRoot);
});

test('pdf-reader: 翻页自动持久化续读点与精准恢复（整本文档页）', async () => {
  const r = await pdfOps.createPdfDoc(wiki, { docTitle: '翻页测试书', dataB64: 'JVBERi0xLjQK' });
...
test('pdf-reader: 续读点页码越出存量节区间也精确恢复（连续跨节阅读兼容）', async () => {
  ...
  const docTitle = 'Tidme/Docs/越区间兼容书';
  const pdfTitle = 'Tidme/Assets/越区间兼容书';
  const docId = 'dout1';
  wiki.addTiddler({
    title: docTitle,
    tags: ['tidme-doc'],
...
test('pdf-reader: 框选图片制卡与 buildImageQA 字段治理（纯净短标题+无Base64污染+即时答案）', async () => {
  const r = await pdfOps.createPdfDoc(wiki, { docTitle: '图片制卡测试书', dataB64: 'JVBERi0xLjQK' });
...
test('pdf: 续读点持久化 & 回原文锚点（整本文档页）', async () => {
  const b = await pdfOps.createPdfDoc(wiki, { docTitle: '精确定位测试书', dataB64: 'fake-bytes' });
...
test('pdf: 阅读条栏推进（▶ 下一节）续读点携带目标节起始页（存量分节书兼容，不再丢失阅读位置）', async () => {
  const r = await pdfOps.createPdfDoc(wiki, { docTitle: '页码携带测试书', dataB64: 'JVBERi0xLjQK' });
...
test('pdf: 学习模式「完成，下一张」续读点指向下一节并携带起始页（存量分节书兼容）', async () => {
  const r = await pdfOps.createPdfDoc(wiki, { docTitle: '学习推进页码书', dataB64: 'JVBERi0xLjQK' });
  const sec1 = `${r.docTitle}/01 扉页`;
  const sec2 = `${r.docTitle}/02 序言`;
  for (const [i, st] of [sec1, sec2].entries()) {
    wiki.addTiddler({
...
test('core/doc-ops: docReadingProgress 真实阅读进度计算（解决 PDF 进度永久 0/1）', async () => {
  const r = await pdfOps.createPdfDoc(wiki, { docTitle: '进度条测试书', dataB64: 'JVBERi0xLjQK', pagesTotal: 100 });
  // 未读状态（第一页）
  let prog = docOps.docReadingProgress(wiki, r.docId);
  assert.equal(prog.type, 'continuous');
  assert.equal(prog.current, 1);
  assert.equal(prog.total, 100);
  assert.equal(prog.percent, 1);
  assert.equal(prog.doneText, 'p.1/100');

  // 更新续读点到第 35 页
  docOps.saveReadPoint(wiki, r.docId, { t: r.docTitle, s: 'p35' });
  prog = docOps.docReadingProgress(wiki, r.docId);
  assert.equal(prog.current, 35);
  assert.equal(prog.total, 100);
  assert.equal(prog.percent, 35);
  assert.equal(prog.doneText, 'p.35/100');
});

test('core/ns + paths: 统一 Doc 命名空间与 Decks 镜像推导', () => {
