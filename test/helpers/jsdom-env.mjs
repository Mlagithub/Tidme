/*
jsdom-env.mjs — 导入管线（bin/pipeline.cjs）无头运行的 DOM 与依赖注入

- installDom()：把 jsdom 的 DOMParser/XMLSerializer/Node 挂到 globalThis（管线按浏览器全局取用）
...
- loadPipelineBundle()：加载 bin/pipeline.cjs 并完成上述注入（pipeline/split 测试唯一入口）
*/
...
export async function loadPipelineBundle() {
	installDom();
	await installJszip();
