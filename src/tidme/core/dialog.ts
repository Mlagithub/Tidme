/*
core/dialog.ts — 统一确认/提示弹窗（Wave 2：替换全部原生 confirm/alert）
...
function buildModal(doc: Document, opts: { title?: string; message: string; okLabel: string; danger?: boolean; withCancel: boolean },
	resolve: (v: boolean) => void): HTMLElement {
	...
	const okBtn = el(doc, "button", "tm-card-modal-btn tm-card-modal-submit" + (opts.danger ? " tm-dialog-danger" : ""),
		opts.okLabel);
...
export function confirmDialog(doc: Document, opts: ConfirmOptions): Promise<boolean> {
	return new Promise((resolve) => {
		buildModal(doc, {
			...
			okLabel: opts.confirmLabel || "确定", danger: opts.danger, withCancel: true
		}, resolve);
	});
}
...
export function alertDialog(doc: Document, opts: { title?: string; message: string; closeLabel?: string }): Promise<void> {
	return new Promise((resolve) => {
		buildModal(doc, {
			title: opts.title, message: opts.message,
			okLabel: opts.closeLabel || "知道了", withCancel: false
		}, () => resolve());
	});
}
