  ...
  class SettingsWidget extends Widget {
    ...
    build() {
      ...
      const groups: form.SettingGroup[] = [
        ...
        {
          id: 'daily-limits',
          title: l('settings.limits.title', 'Daily Limits & Learning Day'),
          subtitle: l('settings.limits.subtitle', 'New/review caps, rollover hour, learn ahead, burying'),
          items: [
            {
              id: 'new-per-day',
              title: l('settings.limits.new', 'New Cards per Day'),
              desc: l('settings.limits.new.desc', 'Introduced new cards per learning day; 0 = unlimited (default 20)'),
              type: 'number',
              min: 0,
              max: 9999,
              step: 1,
              getValue: () => config.readNewPerDay(wiki),
              setValue: (v: number) => config.writeNewPerDay(wiki, v),
            },
            {
              id: 'reviews-per-day',
              title: l('settings.limits.reviews', 'Reviews per Day'),
              desc: l('settings.limits.reviews.desc', 'Due reviews per learning day; 0 = unlimited (default 200)'),
              type: 'number',
              min: 0,
              max: 9999,
              step: 1,
              getValue: () => config.readReviewsPerDay(wiki),
              setValue: (v: number) => config.writeReviewsPerDay(wiki, v),
            },
            {
              id: 'limits-suppress-new',
              title: l('settings.limits.suppress', 'Reviews Suppress New Cards'),
              desc: l('settings.limits.suppress.desc', 'New cards consume the review budget: when the review cap is used up, new cards stop'),
              type: 'switch',
              getValue: () => config.readLimitsSuppressNew(wiki),
              setValue: (v: boolean) => config.writeLimitsSuppressNew(wiki, v),
            },
            {
              id: 'rollover-hour',
              title: l('settings.limits.rollover', 'Learning Day Starts At (Hour)'),
              desc: l('settings.limits.rollover.desc', 'Local hour when the learning day rolls over (Anki default 4). Daily counts reset at this hour'),
              type: 'number',
              min: 0,
              max: 23,
              step: 1,
              getValue: () => config.readRolloverHour(wiki),
              setValue: (v: number) => config.writeRolloverHour(wiki, v),
            },
            {
              id: 'learn-ahead',
              title: l('settings.limits.learnAhead', 'Learn Ahead Limit (Minutes)'),
              desc: l('settings.limits.learnAhead.desc', 'Show in-session learning cards up to N minutes early when the queue is empty; 0 = wait strictly'),
              type: 'number',
              min: 0,
              max: 1440,
              step: 1,
              getValue: () => config.readLearnAheadMinutes(wiki),
              setValue: (v: number) => config.writeLearnAheadMinutes(wiki, v),
            },
            {
              id: 'bury-siblings',
              title: l('settings.limits.bury', 'Bury Siblings'),
              desc: l('settings.limits.bury.desc', 'After grading a card, hide its same-source siblings until the next learning day (cards in learning steps are never buried)'),
              type: 'switch',
              getValue: () => config.readBurySiblings(wiki),
              setValue: (v: boolean) => config.writeBurySiblings(wiki, v),
            },
          ],
        },
        {
