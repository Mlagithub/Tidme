...
export { bookCardsRoot, bookRoot, insertedSectionTitle, joinPath, leafIdOf, sectionLeaf, sectionPath, slugify } from '$:/plugins/keepone/tidme/core/paths';
...
async function importTextBytes(bytes: Uint8Array, fileName: string, options: ImportOptions): Promise<ImportResult> {
  ...
  const type = /\.(md|markdown)$/.test(ext)
    ? 'text/markdown'
    : /\.html?$/.test(ext)
    ? 'text/html'
    : 'text/plain';
