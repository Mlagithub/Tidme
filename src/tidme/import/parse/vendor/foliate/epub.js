/*
 vendored from foliate-js@1.0.1 (https://github.com/johnfactotum/foliate-js, MIT License —
 见同目录 LICENSE；npm 包本体随 dependencies 安装以便比对更新）。本目录两个文件只被
 esbuild 打包进导入解析 bundle，不作为独立 TW 模块产出（无 .meta）。
 改动：文件头追加本地补丁层（下段 shim，非上游代码），其余保持上游原样，升级时整文件替换。
*/

// ---- 本地补丁层（Tidme 专属，非上游代码）----
// TW 启动沙箱的全局没有 EventTarget/CustomEvent，而上游 `class MediaOverlay extends
// EventTarget` 在模块顶层求值即崩。本插件只用 EPUB 文本解析（EPUB.init + createDocument），
// MediaOverlay/Loader 的事件面不可达，最小 no-op 实现即可。
// 写法约束：必须用 `var X = globalThis.X || ...` 的**模块局部绑定**，不能给 globalThis
// 赋值——TW Node 沙箱（boot.js globalCheck）会把沙箱全局上的新增属性按「全局赋值」告警；
// var 只落在模块包装函数作用域里，且 `extends EventTarget` 的标识符解析照样命中。
// 浏览器真实运行时有原生实现，这里直接引用原值，行为不变。升级 vendored 文件时需保留本段。
var EventTarget = globalThis.EventTarget || class EventTarget {
  addEventListener() {}
  removeEventListener() {}
  dispatchEvent() {
    return true;
...
var CustomEvent = globalThis.CustomEvent || class CustomEvent {
...
const getMetadata = opf => {
  ...
  const dc = Object.fromEntries(
    Object.entries(Object.groupBy(els.dc || [], el => el.localName))
...
class Loader {
  ...
  async loadReplaced(item, parents = []) {
    ...
    if ([MIME.XHTML, MIME.HTML, MIME.SVG].includes(mediaType)) {
      ...
      if (
        mediaType === MIME.XHTML && (doc.querySelector('parsererror') ||
  ...
  async replaceCSS(str, href, parents = []) {
    const replacedUrls = await replaceSeries(str, /url\(\s*["']?([^'"\n]*?)\s*["']?\s*\)/gi, (_, url) =>
      this.loadHref(url, href, parents)
        .then(url => `url("${url}")`));
...
${doc.querySelector('parsererror').innerText}`);
  ...
  async init() {
    ...
    this.#loader = new Loader({
      ...
      loadBlob: uri =>
        Promise.resolve(this.loadBlob(uri))
    ...
    if (displayOptions) {
      ...
      if (displayOptions.openToSpread === 'false') {
        this.sections
          .find(section => section.linear !== 'no').pageSpread ??= this.dir === 'rtl' ? 'left' : 'right';
      }
    }
    return this;
  }
