...
async function importDeck(pluginFile, wikiFolder) {
  ...
  if (Array.isArray(pluginData)) {
    const plugin = pluginData.find(function(t) {
      return t.title && t.title.startsWith('$:/plugins/');
    });
    if (!plugin) {
  ...
  try {
    ...
    const deckTiddlers = Object.keys(allTiddlers).filter(function(t) {
      return t.startsWith('$:/Deck/');
    });
    let deckInfos = [];
    ...
    console.log(`🔍 动态识别到 ${deckInfos.length} 个 Deck 牌组: ${
      deckInfos.map(function(d) {
        return d.deckName;
      }).join(', ')
    }`);
    ...
    for (const info of deckInfos) {
      ...
      for (const title of allCardTitles) {
        ...
        const phonetic = (wordData && wordData.content && wordData.content.word && wordData.content.word.content)
          ? (wordData.content.word.content.usphone
            ? `/${wordData.content.word.content.usphone}/`
            : (wordData.content.word.content.ukphone ? `/${wordData.content.word.content.ukphone}/` : ''))
          : '';
