...
const fs = require("fs");
const imp = JSON.parse(fs.readFileSync("out-m2/$__plugins_tidme_import.json", "utf8"));
const read = JSON.parse(fs.readFileSync("out-m2/$__plugins_tidme_read.json", "utf8"));

const it = JSON.parse(imp.text).tiddlers;
const w = it["$:/plugins/tidme/import/widgets/section.js"].text;
...
const rb = JSON.parse(read.text).tiddlers["$:/plugins/tidme/read/ui/ViewTemplate/body"].text;
