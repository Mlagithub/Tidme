...
背景：早期导入产物只有 tidme.breadcrumb（"书名 › 章 › 节"），无权威 parent 链。
本脚本按 breadcrumb 反推层级：
  - 节 的父 = breadcrumb 去掉末段后对应的 tiddler（按 tidme.breadcrumb 精确匹配）
  - 找不到则父 = 文档页（breadcrumb 首段，tidme-doc 标签）
...
for (const title of sections) {
  ...
  if (!parent) {
    // 回退：文档页 = 首段 + tidme-doc 标签
    parent = wiki.filterTiddlers(`[title[${segs[0]}]tag[tidme-doc]]`)[0] || segs[0];
  }
